...
def create_partialindex(option):
    ...
    if os.path.isdir("INDEX-4"):
        shutil.rmtree("INDEX-4")
        print "Removed Existing INDEX-4"



    os.mkdir("INDEX-4")
    global output
    output = open("./INDEX-4/partialindex(NSST).json",'w')
    ...
    with open("./INDEX-4/catalogue(NSST).json",'w') as offset:
        json.dump(term_offset,offset)

    doc_map["total"] = total_length

    with open("./INDEX-4/document_mapping(NSST).json",'w') as docmap:
        json.dump(doc_map,docmap)

    with open("./INDEX-4/term_mapping(NSST).json",'w') as termmap:
